...
const Header = ({ setCursorVariant, handleCursorChange }) => {
  ...
  return (
    <>
      ...
      <motion.header 
        className={`header ${isCompact ? 'header-hidden' : ''}`}
        initial={false}
        animate={{ 
          y: isCompact ? -100 : 0,
        ...
        <nav className={`nav ${menuOpen ? 'open' : ''}`}>
          {navItems.map((item, index) => (
            <Link
              ...
              onMouseEnter={() => handleNavMouseEnter(index, false)}
              onMouseLeave={() => handleNavMouseLeave(false)}
            ...
            >
              ...
              <AnimatePresence mode="wait">
                {(activeIndex === index && hoveredIndex !== index) || hoveredIndex === index ? (
                  ...
                  <span className="nav-dot-placeholder" />
                )}
              </AnimatePresence>
      ...
      {/* Compact Bubble Menu - visible when scrolled */}
      <AnimatePresence>
        {isCompact && (
          <motion.nav
